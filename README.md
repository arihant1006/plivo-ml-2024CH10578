...
**Result: dev bpb 2.3718 (baseline) → 2.1738 (final), an 8.35% relative improvement**, from a
hand-rolled BPE tokenizer, a SwiGLU MLP, and learning-rate tuning + cosine decay. Full
experimental reasoning (10 runs, hypothesis/result/conclusion for each) is in `RUNLOG.md`; the
condensed final configuration and rationale is in `NOTES.md`; a consolidated, human-readable
report is in `SUMMARY.html`.
...
| File | What it is |
|---|---|
| `model.py` | GPT architecture (final: 4-layer/4-head/160-dim, SwiGLU MLP, untied, 1,499,408 params) |
...
| `ckpt.pt` | Final checkpoint (2,000 steps, dev bpb 2.1738) |
...
- **SwiGLU MLP** instead of the standard 4x-GELU MLP, at matched parameter count.
- **Learning rate:** constant 3e-4 → cosine decay 1e-3 → 1e-4, no warmup. The baseline's loss
  ...
  strong, 2e-3 decisively unstable) before adding decay.
- **Weight tying, capacity increase, and GPT-2-style init were each tried and rejected** —
  all three made dev bpb worse under this specific 2000-step budget. See RUNLOG.md for why.
